...
class DataSourceEntryChangeForMapBuilderListener
{
    /** @var LoggerInterface */
    protected $logger;
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $importHistoryRepository = $em->getRepository(ImportHistory::class);
        $importHistories = $importHistoryRepository->getImportHistoryByDataSourceEntryWithoutDataSet($dataSourceEntry);

        if (!is_array($importHistories) || count($importHistories) < 1) {
            return;
        }
