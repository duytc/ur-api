...
class DataSourceController extends RestControllerAbstract implements ClassResourceInterface
{
     ...
     *
     * @Rest\View(serializerGroups={"datasource.edit", "dataSourceIntegration.summary", "integration.summary", "user.summary"})
     *
     * @Rest\QueryParam(name="page", requirements="\d+", nullable=true, description="the page to get")
     *
     * @ApiDoc(
    ...
    public function postUploadForDetectedFieldsAction(Request $request, $id)
    {
        ...
        $result = $importService->detectFieldsFromFiles($files, $dirItem, $dataSource);
        if (!is_array($result)) {
