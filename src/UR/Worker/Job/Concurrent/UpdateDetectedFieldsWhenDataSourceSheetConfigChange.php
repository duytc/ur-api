...
class UpdateDetectedFieldsWhenDataSourceSheetConfigChange implements LockableJobInterface
{
    const JOB_NAME = 'updateDetectedFieldsWhenDataSourceSheetConfigChange';

    const PARAM_KEY_ENTRY_ID = 'entryId';
    const PARAM_KEY_DATA_SOURCE_ID = 'data_source_id';
    const PARAM_KEY_DELETED_SHEETS = 'deletedSheets';
    ...
    public function run(JobParams $params)
    {
        ...
        $oldSheets = $params->getRequiredParam(self::PARAM_KEY_DELETED_SHEETS);
        ...
        try {
            ...
            $dataSourceOldSheetFile = $this->importService->getDataSourceFile($dataSourceTypeExtension, $dataSourceEntry->getPath(), $oldSheets);
            $oldDeletedFields = $this->importService->getNewFieldsFromFiles($dataSourceOldSheetFile);
            ...
            $detectedFields = $this->importService->detectFieldsForDataSource($oldDeletedFields, $detectedFields, ImportService::ACTION_DELETE);
